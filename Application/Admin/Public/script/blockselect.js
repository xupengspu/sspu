...
(function (main) {
    ...
    blocklist.addListener = function () {
        ...
        $("#confirm").click(function () {
            var article_ids = [];
            $(".check-article:checked").each(function () {
                //由于复选框一般选中的是多个,所以可以循环输出
                article_ids.push($(this).attr('data-id'));
            });

            $.ajax({
                url: '/admin/menu/addcontent',
                type: 'POST',
                dataType: 'json',
                data: {'menu_id': $('#menu_id').val(), 'article_ids': article_ids},
                success: function (resp) {
                    //查询成功
                    if (resp.code == 0) {
                        var index = parent.layer.getFrameIndex(window.name); //先得到当前iframe层的索引
                        parent.layer.close(index);
                        window.parent.menu_content.callback();
                    }
                }
            });
        });
